...
import { Sparkles, Mail, Lock, Globe, User } from 'lucide-react';
...
export default function Signup() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', background: semantic.bg.primary }}>
      ...
      <div style={{ flex: 1, ...flexCenter, padding: '2rem', position: 'relative', zIndex: 10 }}>
        ...
        <div style={{ width: '100%', maxWidth: '400px' }}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <p style={{ color: semantic.text.secondary }}>Start your free trial today.</p>
          </div>
          ...
          <div style={{ marginTop: '2rem', textAlign: 'center', fontSize: typography.fontSize.sm, color: semantic.text.secondary }}>
            ...
            <Link href="/login" style={{ color: semantic.text.primary, fontWeight: 500, textDecoration: 'none' }}>
              Log in
            </Link>
          </div>
        </div>
      ...
      {/* Right side - Visual */}
      <div className="signup-visual-panel" style={{ flex: 1, background: semantic.bg.secondary, position: 'relative', overflow: 'hidden' }}>
      ...
      <style>{`
        .signup-visual-panel { display: none; }
        @media (min-width: 1024px) {
          .signup-visual-panel { display: block !important; }
        }
      `}</style>
    </div>
  );
